...
It creates a temporary .csproj file and executes [dotnet publish](🔗), which uses .NET SDK to compile the script.
...
- `uac`. By default programs run not as administrator (unlike when launched from admin LibreAutomate) and therefore can't automate admin windows etc. See [UAC](🔗).
...
To get program/OS/computer info can be used classes `Au.script`, `Au.process`, `Au.uacInfo`, `Au.osVersion`, `Au.folders`, [Environment](🔗), `RuntimeInformation`, `OperatingSystem`.
