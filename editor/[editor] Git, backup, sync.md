...
You need a [GitHub](🔗) account, and a private repository there. It's free.

1. In your GitHub account [create new repository](🔗). It must be private and empty (no readme etc). Copy its URL.
...
Git can be used via command line. Or you can install a [Git GUI client](🔗). Try [GitHub Desktop](🔗), it's free and easy to use. In its **File** menu select **Add local repository** and add the workspace folder. If you need a GUI client with more features, try TortoiseGit.
...
Git ignores (does not backup etc) files and directories specified in file named `.gitignore` in the workspace directory. The Git setup dialog creates it if missing. You can edit it ([how](🔗)). Default text:
