...
- *recorded*  (`IEnumerable<uint>`):
  ...
  The first distance is from mouse position before the first movement; at run time it will be distance from`Au.mouse.lastXY`.
  To create`uint` value from distance *dx* *dy* use `Au.More.Math2.MakeLparam` and cast to `uint`.
- *withSleepTimes*  (`bool`):
