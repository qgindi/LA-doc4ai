...
On Windows 8.1 returns`Au.More.Dpi.Awareness.PerMonitor` if *w* is of this process; else uses API `GetProcessDpiAwareness`, which is slower and less reliable.
On Windows 7 and 8.0 always returns`Au.More.Dpi.Awareness.System`, because there are no Windows API.