...
- *wndProc*  (`Au.Types.WNDPROC`):
  ...
  Delegate of a window procedure. See [Window Procedures](🔗).
  
  Use `null` when you need a different delegate (method or target object) for each window instance; create windows with `Au.More.WndUtil.CreateWindow` or `Au.More.WndUtil.CreateMessageOnlyWindow`.
  If not`null`, it must be a static named method; create windows with any other function, including API `CreateWindowEx`.
- *etc*  (`Au.Types.RWCEtc`):
...
If called next time for the same window class, does nothing if*wndProc* is equal to the previous (or both `null`). Then ignores *etc*. Throws exception if different.
Thread-safe.
