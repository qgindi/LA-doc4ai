...
- *w*  (`Au.wnd`):
  Window or control.
- *cnChild*  (`string`):
  Child window class name. Format: [wildcard expression](🔗). If used, gets COM object from the first found child or descendant window where it succeeds. If `null`, gets COM object from *w*.
- *dontThrow*  (`bool`):
