...
Low level hooks may be occasionally disabled by the OS or other hooks. Workaround - call this function eg every 10 s in same thread. For example use `Au.timer`. Don't call too frequently, eg every 1 s.
