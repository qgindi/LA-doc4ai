...
The memory is unmanaged and will not be freed automatically. Always call`Au.More.MemoryUtil.Free` when done. Call `ReAlloc` or `Au.More.MemoryUtil.FreeAlloc<T>` if need to resize.