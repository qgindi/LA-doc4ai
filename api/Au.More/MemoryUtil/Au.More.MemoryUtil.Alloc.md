...
Calls API `HeapAlloc`.
The memory is unmanaged and will not be freed automatically. Always call`Au.More.MemoryUtil.Free` when done. Call `Au.More.MemoryUtil.ReAlloc<T>` or `Au.More.MemoryUtil.FreeAlloc<T>` if need to resize.
