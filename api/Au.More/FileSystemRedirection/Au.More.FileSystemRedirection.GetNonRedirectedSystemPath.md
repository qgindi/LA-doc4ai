...
If `Au.osVersion.is32BitProcessAnd64BitOS` is `true` and *path* starts with `Au.folders.System`, replaces that path part with `Au.folders.SystemX64`.
It disables redirection to`Au.folders.SystemX86` for that path.
