...
The key must not be an "extended key". See `Au.Types.KKeyScan` example code. Don't use this flag with `NumpadX` flags.

### `ExtendedYes`

The key must be an "extended key". See `Au.Types.KKeyScan` example code. Don't use this flag with `NumpadX` flags.
