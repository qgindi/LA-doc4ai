...
- *later*  (`Au.Triggers.TWLater`):
  ...
  For example, to be notified when the window is closed or renamed, specify`later: TWLater.Destroyed | TWLater.Name`.
  When a "later" event occurs, the trigger action is executed. The`Au.Triggers.WindowTriggerArgs.Later`property then is that event; it is 0 when it is the primary trigger.
