...
The same as`Au.Triggers.TWEvent.ActiveOnce`, but windows created before calling `Au.Triggers.ActionTriggers.Run` are ignored.
...
The same as`Au.Triggers.TWEvent.VisibleOnce`, but windows created before calling `Au.Triggers.ActionTriggers.Run` are ignored.
