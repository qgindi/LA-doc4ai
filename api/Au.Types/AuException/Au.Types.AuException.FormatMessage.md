...
If*appendMessage* is `null`, uses `lastError.messageFor(NativeErrorCode)` if `Au.Types.AuException.NativeErrorCode`not 0.
If then*appendMessage* is not empty, appends `" " + appendMessage`.
Also appends`InnerException.Message` in new tab-indented line if `System.Exception.InnerException` is not `null`.