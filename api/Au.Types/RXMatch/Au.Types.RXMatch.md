...
Used with`Au.regexp` class functions and `System.String` extension methods like `Au.Types.ExtString.RxMatch`.
...
Also contains an array of group matches, as`Au.Types.RXGroup`. Groups are regular expression parts enclosed in `()`, except `(?...)`.
