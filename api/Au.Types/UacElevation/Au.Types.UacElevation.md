...
Runs as administrator (`High` or `System` integrity level, see `Au.Types.UacIL`), and UAC is not turned off. Also known as "elevated".

### `Limited`

Runs as standard user (`Medium`, `UIAccess` or `Low` integrity level, see `Au.Types.UacIL`) in administrator user session (because of UAC).

### `Unknown`

Failed to get. Normally it never happens.