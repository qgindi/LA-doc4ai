...
Extract file path or script path from item action code (for example `Au.run.it` or `Au.script.run`argument) and use icon of that file or script.
...
Gets path from code that contains a string like `@"c:\windows\system32\abc.exe"` or `@"%folders.System%\abc.exe"` or URL/shell or `@"\folder\script.cs"`.
Also supports code patterns like`folders.System + "abc.exe"`, `folders.shell.RecycleBin`.
