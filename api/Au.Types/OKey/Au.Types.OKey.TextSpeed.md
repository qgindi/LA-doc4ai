...
How long to wait (milliseconds) between pressing and releasing each character key. Used by `Au.keys.sendt`. Also by `Au.keys.send` and similar functions for `"!text"`arguments.
Default: 0.
