...
When copying or pasting text, and `Au.Types.OKey.RestoreClipboardAllFormats` is `true`, do not restore clipboard data of these formats.
Default:`null`.
...
There are several kinds of clipboard formats - registered, standard, private and display. Only registered formats have string names. For standard formats use API constant names, like `"CF_WAVE"`. Private, display and metafile formats are never restored.
