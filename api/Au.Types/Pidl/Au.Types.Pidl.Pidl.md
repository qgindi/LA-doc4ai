...
- *pidl*  (`nint`):
  `ITEMIDLIST*`.
  It can be created by any API that creates`ITEMIDLIST`. They allocate the memory with API `CoTaskMemAlloc`. This variable will finally free it with `System.Runtime.InteropServices.Marshal.FreeCoTaskMem` which calls API `CoTaskMemFree`.
