...
Some constructors support Windows API error code. Then `Au.Types.AuWndException.Message`also will contain its error description.
If error code`ERROR_INVALID_WINDOW_HANDLE`, `Au.Types.AuWndException.Message`also depends on whether the window handle is 0.
