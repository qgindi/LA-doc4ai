...
Then, if you need error info, you can use`Au.lastError`. If the file/directory does not exist, it will return `ERROR_FILE_NOT_FOUND` or `ERROR_PATH_NOT_FOUND` or `ERROR_NOT_READY`.
...
Pass path to the API as it is, without any normalizing and validating.