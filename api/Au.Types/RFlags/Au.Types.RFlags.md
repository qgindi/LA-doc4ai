...
If started new process, get process handle (`Au.Types.RResult.ProcessHandle`).

### `ShowErrorUI`

Show error message box if fails, for example if file not found.
Note: this does not disable exceptions. To avoid exceptions use try/catch or`Au.run.itSafe`.
