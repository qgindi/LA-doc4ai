...
Let `Au.Types.FEFile.Name` be path relative to the specified directory path. Like `@"\name.txt"` or `@"\subdirectory\name.txt"` instead of `"name.txt"`.

### `OnlyFiles`

Get only files and not subdirectories.
Note: the*dirFilter* callback function is called just to ask whether to include children.

### `RecurseNtfsLinks`

Also enumerate target directories of NTFS links, such as symbolic links and mount points. Use with `AllDescendants`.
...
Don't call `Au.pathname.normalize` and don't throw exception for non-full path.