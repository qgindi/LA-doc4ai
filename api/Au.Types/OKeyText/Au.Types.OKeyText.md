...
All apps support it.
If a character cannot be simply typed with the keyboard using current keyboard layout, sends it like with the`Characters` option.
...
Other options are unreliable when text length is more than 4000 and the target app is too slow to process sent characters. Then`Au.Types.OKey.TextSpeed`can help.
