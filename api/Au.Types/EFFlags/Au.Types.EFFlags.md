...
When control class or id is specified in the*prop*argument, this flag is applied to these controls. Not applied to other controls.
...
Without this flag skips`MENUITEM` descendant elements (for speed), unless *role* argument is `MENUITEM` or `MENUPOPUP` or searching in web page.
...
Some windows have child controls that belong to a different process or thread than the window. For example the Windows Task Scheduler window. When searching in such windows, the default search method is not used when searching in these controls. Workaround - find the control (`Au.wnd.Child`etc) and search in it.
Don't need this flag when searching in elm (then it is inherited from the elm variable).
See also:`Au.elm.MiscFlags`.

### `Reverse`

Search in reverse order. It can make faster.
When control class or id is specified in the*prop* argument, controls are searched not in reverse order. Only UI elements in them are searched in reverse order.
...
UI elements found with this flag never have`HtmlX` properties, but can have `UiaX`properties.
