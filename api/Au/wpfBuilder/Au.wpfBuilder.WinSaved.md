...
Calls `Au.More.WndSavedRect.Restore`.
Call this function before showing the window. Don't change location/size-related window properties after that.
If you use`Au.wpfBuilder.WinSize`, call it before. It is used if size is still not saved. The same if you set window position or state.
