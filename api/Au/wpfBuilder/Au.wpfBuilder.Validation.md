...
The callback function will be called when clicked button **OK** or **Apply** or a button added with flag `Au.Types.WBBFlags.Validate`.
If it returns a non-`null` string, the window stays open and button's *click* callback not called. The string is displayed in a tooltip.
