...
Calls `Au.wpfBuilder.End`, sets `System.Windows.Window.Owner` and calls `System.Windows.Window.ShowDialog`.
You can instead call these functions directly. Or call`System.Windows.Window.Show` to show as non-modal window, ie don't wait. Or add `Au.wpfBuilder.Panel` to some container window or other element, etc.