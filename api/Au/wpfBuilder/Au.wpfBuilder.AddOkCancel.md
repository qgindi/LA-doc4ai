...
Sets properties of **OK**/**Cancel** buttons so that click and `Enter`/`Esc` close the window; then `Au.wpfBuilder.ShowDialog` returns `true` on **OK**, `false` on **Cancel**.
See also event`Au.wpfBuilder.OkApply`.
