...
Adds inlines (text, formatted text, hyperlinks, images, etc) to the last added element (eg `System.Windows.Controls.TextBlock`).
...
- *text*  (`Au.wpfBuilder`.[InterpolatedString](🔗)):
  ...
  The `text`in above examples can contain nested tags and elements.
  The`<a>` tag creates an image hyperlink if `text` is `{image}`, where image is a `System.Windows.UIElement` with image (see `Au.More.ImageUtil.LoadWpfImageElement`) or `System.Windows.Media.ImageSource` (see `Au.More.ImageUtil.LoadWpfImage`, `Au.icon.ToWpfImage`).
...
- `NotSupportedException`:
  Unsupported type or content of the last added element.
- `ArgumentException`:
...
- `System.Windows.Controls.ContentControl` (eg `System.Windows.Controls.Label` or `System.Windows.Controls.Button`) or `System.Windows.Controls.HeaderedContentControl` (eg `System.Windows.Controls.GroupBox`) - creates new `System.Windows.Controls.TextBlock` with inlines and sets its `Content` or `Header` property. Or uses existing `TextBlock`.
...
- `RichTextBox`, `FlowDocumentScrollViewer` or `FlowDocumentReader` with non-`null` `Document` - adds single `System.Windows.Documents.Paragraph` with inlines. Or uses existing single `Paragraph`.
...
<a href='https://www.example.com'>example.com</a> <b><a href='mspaint.exe'>mspaint</a></b>
