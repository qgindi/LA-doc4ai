...
```csharp
public static trayIcon trayIcon(int delay = 500, Action<trayIcon> init = null, Action<trayIcon, popupMenu> menu = null, string f_ = null)
```
...
- *f_*  (`string`):
  [Caller info parameter](🔗). Don't use. Or set = `null` to disable script editing via the tray icon.

##### Returns

`Au.trayIcon`

A `trayIcon` variable that will manage the icon.
