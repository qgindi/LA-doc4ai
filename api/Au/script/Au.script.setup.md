...
- *exitKey*  (`Au.Types.KKey`):
  ...
  Example:`exitKey: KKey.MediaStop`.
...
- *pauseKey*  (`Au.Types.KKey`):
  ...
  Let `Au.script.pause` pause/resume when this key pressed. Default: `ScrollLock` (`Fn+S`, `Fn+K`or similar).
...
- *f_*  (`string`):
  [Caller info parameter](🔗). Don't use. Or use like `f_: null` to disable script editing via tray icon.
