...
Used in "find" functions. For example in `Au.wnd.find`to compare window name, class name and program.
The "find" function creates a`wildex` instance (which parses the wildcard expression), then calls `Au.wildex.Match` for each item (eg window) to compare some its property text.
