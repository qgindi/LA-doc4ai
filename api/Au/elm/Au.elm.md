...
`elm` functions that get properties don't throw exception when the COM etc method failed (returned an error code of `HRESULT`type).
Then they return`""` (string properties), 0, `false`, `null`or empty collection, depending on return type.
...
These`elm` functions also set the last error code of this thread = the return value (`HRESULT`) of the COM function, and callers can use `Au.lastError`to get it.
If`Au.lastError.code` returns 1 (`S_FALSE`), in most cases it's not an error, just the property is unavailable. On error it will probably be a negative error code.
...
[Known UI element issues in various applications](🔗)
