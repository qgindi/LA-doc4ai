...
Not all UI elements support it. Most UI elements support not all flags. It depends on `Au.Types.EState` `FOCUSABLE`, `SELECTABLE`, `MULTISELECTABLE`, `EXTSELECTABLE`, `DISABLED`.
Many UI elements have bugs, especially with flag`TAKEFOCUS`. More bugs when the UI element has been found with flag `Au.Types.EFFlags.NotInProc`.