...
- *raw*  (`bool`):
  Don't DPI-scale. When the element is in a DPI-scaled/virtualized window (see `Au.More.Dpi.IsWindowVirtualized`), the raw rectangle may not match the visible rectangle.
...
Most but not all UI elements support this property.
Uses`Au.elm.GetRect(out RECT, bool)` and `Au.wnd.MapScreenToClient`.