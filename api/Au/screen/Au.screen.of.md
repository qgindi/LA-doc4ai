...
- *defaultScreen*  (`Au.Types.SODefault`):
  Enum: Zero, Primary, Nearest.
- *lazy*  (`bool`):
  
  Create variable with `Au.screen.LazyFunc`that later will get screen handle.
  Other ways to create lazy:
  
  - use `Au.wndFinder`. Example: `screen.of(new wndFinder("* window name"))`.
  - use constructor. Example: `new screen(() => screen.of(wnd.findFast(cn: "classname")))`.
