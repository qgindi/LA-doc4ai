...
- *inScreen*  (`bool`):
  Get rectangle in screen coordinates; like `Au.wnd.GetWindowAndClientRectInScreen`but faster.
  If`false` (default), calls API `GetClientRect`; the same as `Au.wnd.ClientRect`.
