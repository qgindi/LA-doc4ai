...
- *dontThrowIfClosed*  (`bool`):
  ...
  In such case the callback function must return a non-default value, like in examples with`Au.wnd.IsAlive`. Else exception is thrown (with a small delay) to prevent infinite waiting.
