...
Faster than`Au.wnd.find`, which uses API `EnumWindows`.
Finds hidden windows too.
Supports`Au.lastError`.
It is not recommended to use this function in a loop to enumerate windows. It would be unreliable because window positions in the Z order can be changed while enumerating. Also then it would be slower than`Au.wnd.find` and `Au.wnd.findAll`.