...
This is a low-level function. You can instead use`Au.wnd.Name` and `Au.wnd.ControlText`.
...
- *removeUnderlineAmpersand*  (`bool`):
  ...
  Removes only if this is a control (has style`Au.Types.WS.CHILD`).
  Calls`Au.More.StringUtil.RemoveUnderlineChar`.
