...
This function just calls `Au.wnd.Child` and `Au.mouse.postClick`.
...
- *asControl*  (`bool`):
  If `true`, finds/clicks as child control: `mouse.postClick(this.Child(1, name, roleCN ?? "*Button*"));`.
  If`false`, finds/clicks as UI element: `this.Elm[roleCN ?? "BUTTON", name].Find(1).Invoke();`.
  Default is`false`; it's slower but works with more windows.
- *roleCN*  (`string`):
  UI element role or control class name (if *asControl* `true`). String format: [wildcard expression](🔗). Default role is `"BUTTON"`, class name `"*Button*"`.
