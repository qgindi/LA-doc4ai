...
- *bind*  (`object`[]):
  ...
  Read about SQL parameters in SQLite website. Supported types:`Au.sqliteStatement.BindAll`. Example: `Au.sqlite`.
...
- *bind*  (`Action<Au.sqliteStatement>`):
  Callback function that should bind (`Au.sqliteStatement.Bind`) values to `?` characters in *sql*.
