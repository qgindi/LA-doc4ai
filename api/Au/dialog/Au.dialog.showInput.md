...
- *buttons*  (`string`):
  Buttons. List of names or `"id name"`. Example: `"1 OK|2 Cancel|10 Browse..."`. See `Au.dialog.Buttons`.
  Note: this function returns`true`only when clicked button with id 1.
  Usually custom buttons are used with*onButtonClick* function, which for example can get button id or disable closing the dialog.
