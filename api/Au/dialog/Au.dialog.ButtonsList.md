...
- *buttons*  (`Au.Types.Strings`):
  List of button names. Can be string like `"One|Two|..."` or `string[]` or `List<string>`.
  Button ids will be 1, 2, ... . Default button will be 1, unless changed with`Au.dialog.Default`.
  Unlike`Au.dialog.Buttons`, this function does not allow to specify button ids; also all specified buttons will be custom buttons, even if named like `"OK"`.
- *asCommandLinks*  (`bool`):
