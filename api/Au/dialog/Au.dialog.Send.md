...
Example: `d.Send.Close();`.
Example:`d.Send.ChangeText2("new text", false);`.
Example:`d.Send.Message(DNative.TDM.CLICK_VERIFICATION, 1);` .
...
Can be used in dialog event handlers. Also can be used in another thread, for example with`Au.dialog.showNoWait` and `Au.dialog.showProgress`.