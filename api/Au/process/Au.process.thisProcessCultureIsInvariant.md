...
If your app doesn't want to use current culture (default in .NET apps), it can set these properties = `System.Globalization.CultureInfo.InvariantCulture` or set this property = `true`.
...
Also, there is a bug in "number to/from string" functions in some .NET versions with some cultures: they use wrong minus sign, not ASCII`'-'`which is specified in Control Panel.
The default compiler sets this property =`true`; as well as `Au.script.setup`.