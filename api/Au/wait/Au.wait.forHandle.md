...
On timeout returns 0 if*timeout* is negative; else exception.
...
Uses API `WaitForMultipleObjectsEx` or `MsgWaitForMultipleObjectsEx`. Alertable.
Does not use`Au.More.WaitLoop`, `Au.Types.Seconds.Period` and `Au.Types.Seconds.MaxPeriod`.