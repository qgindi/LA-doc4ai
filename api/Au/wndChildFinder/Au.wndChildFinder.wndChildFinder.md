...
- *name*  (`string`):
  ...
  By default to get control names this function uses `Au.wnd.Name`.
  ...
  - `"***wfName "` - use .NET Forms control name (see `Au.More.WinformsControlNames`). Slower and can fail because of [UAC](🔗).
- *cn*  (`string`):
  Control class name.
  String format:[wildcard expression](🔗).
  `null` means "can be any". Cannot be `""`.
- *flags*  (`Au.Types.WCFlags`):
  Enum: HiddenToo, DirectChild.
- *id*  (`int?`):
