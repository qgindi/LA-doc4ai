...
Used like`m.Submenu("Example", m => { /* add submenu items */ });`.
...
The caller creates the submenu (creates the `Au.popupMenu` object and adds items) and can reuse it many times. Other overload does not allow to create `Au.popupMenu`and reuse same object.
