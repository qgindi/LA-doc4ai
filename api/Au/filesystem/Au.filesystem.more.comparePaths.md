...
Before comparing, calls `Au.filesystem.more.getFinalPath`, therefore paths can have any format.
Example:`@"C:\Test\"` and `@"C:\A\..\Test"`are equal.
Example:`@"C:\Test\file.txt"` and `"file.txt"` are equal if the file is in `@"C:\Test` and `@"C:\Test`is current directory.
Example:`@"C:\Temp\file.txt"` and `"%TEMP%\file.txt"` are equal if `TEMP` is an environment variable = `@"C:\Temp`.
...
Compares final paths of two existing files or directories to determine equality or relationship.
Also gets final paths (see`Au.filesystem.more.getFinalPath`).
