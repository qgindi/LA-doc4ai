...
Supports `Au.lastError`. If you need exception when fails, instead call `Au.filesystem.getAttributes`.
Always use full path. If not full path: if*useRawPath* is `false` (default), can't find the file; if *useRawPath* is `true`, searches in "current directory".
