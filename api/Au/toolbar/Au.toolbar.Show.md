...
- *screen*  (`Au.screen`):
  Attach to this screen. For example a screen index (0 the primary, 1 the first non-primary, and so on). Example: `screen.index(1)`.
  If not specified, the toolbar will be attached to the screen where it is now or where will be moved later.
  Don't use this parameter if this toolbar was created by`Au.toolbar.AutoHideScreenEdge`, because then screen is already known.
...
Shows the toolbar.
If*ta* is `Au.Triggers.WindowTriggerArgs`, attaches the toolbar to the trigger window.
Else if*ta* != `null`, calls `Au.Triggers.TriggerArgs.DisableTriggerUntilClosed`.
