...
Some functions of this library simply call these API functions and don't throw exception when API fail. For example, most `Au.wnd`property-get functions.
When failed, they return`false`/0/`null`/empty. Then you can use `Au.lastError.code` to get the error code or `Au.lastError.message` to get error text.
