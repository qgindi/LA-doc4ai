...
- *options*  (`Au.Types.OKey`):
  ...
  Uses`Au.Types.OKey.RestoreClipboard`, `Au.Types.OKey.KeySpeedClipboard`, `Au.Types.OKey.NoBlockInput`. Does not use `Au.Types.OKey.Hook`.
...
Sends keys `Ctrl+C`, waits until the focused app sets clipboard data, calls the callback function that gets it, finally restores clipboard data.
