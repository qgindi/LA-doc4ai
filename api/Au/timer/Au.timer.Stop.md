...
Later you can start the timer again (call`Au.timer.After` or `Au.timer.Every`).
...
This function must be called in the same thread as`Au.timer.After` or `Au.timer.Every`.