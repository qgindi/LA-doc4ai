...
Gets match info as`Au.Types.RXMatch`.
...
Returns `true` if string *s*matches this regular expression.
Gets whole match or some group, as`Au.Types.RXGroup` (index, length, value).
...
- *group*  (`int`):
  ...
  See also`Au.regexp.GetGroupNumberOf`.
...
Returns `true` if string span *s*matches this regular expression.
Gets whole match or some group, as`Au.Types.StartEnd`.
