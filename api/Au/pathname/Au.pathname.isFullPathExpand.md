...
- *path*  (`string`):
  ...
  If starts with`'%'` character, calls `Au.pathname.expand` and then `Au.pathname.isFullPath` with expanded environment variables. If it returns `true`, replaces the passed variable with the expanded path string.
- *strict*  (`bool?`):
...
- *path*  (`string`):
  Any string. Can be `null`.
  If starts with`'%'` character, calls `Au.pathname.expand` and then `Au.pathname.isFullPath` with expanded environment variables. If it returns `true`, sets *path2* = the expanded path string.
- *path2*  (`string`):
