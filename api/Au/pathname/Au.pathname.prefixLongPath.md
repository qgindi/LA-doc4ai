...
If *path* is full path (see `Au.pathname.isFullPath`) and does not start with `@"\\?\"`, prepends `@"\\?\"`.
...
- *path*  (`string`):
  Path. Can be `null`.
  Must not start with`"%environmentVariable%"`. This function does not expand it. See `Au.pathname.expand`.
