...
Does not expand environment variables. For it use`Au.pathname.expand` before, or `Au.pathname.normalize`instead. Path that starts with an environment variable here is considered not full path.
Similar to`System.IO.Path.Combine`. Main differences: has some options; supports `null` arguments.
