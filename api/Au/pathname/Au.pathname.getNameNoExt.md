...
The same as `Au.pathname.getName`, just removes extension.
Similar to`System.IO.Path.GetFileNameWithoutExtension`. Some differences: if ends with `'\\'` or `'/'`, gets part before it, eg `"B"` from `@"C:\A\B\"`.
