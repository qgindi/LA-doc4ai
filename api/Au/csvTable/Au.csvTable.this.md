...
The `get` function gets the row array. It's not a copy; changing its elements will change content of this `Au.csvTable`variable.
The`set` function sets the row array. Does not copy the array, unless its `Length` is less than `Au.csvTable.ColumnCount`.
