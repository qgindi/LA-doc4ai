...
- *csv*  (`string`):
  CSV text.
  If rows in CSV text have different field count, the longest row sets the`Au.csvTable.ColumnCount` property and lengths of all row arrays; array elements of missing CSV fields will be `null`.
- *separator*  (`char`):
