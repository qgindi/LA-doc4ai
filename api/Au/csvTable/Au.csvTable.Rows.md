...
It's not a copy; changing its content will change content of this `Au.csvTable`variable.
...
All row arrays have`Length` equal to `Au.csvTable.ColumnCount`, and it must remain so; you can change `Length`, but then need to call `ColumnCount=newLength`.
