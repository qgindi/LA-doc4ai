...
- *flags*  (`Au.Types.RFlags`):
  Enum: ShowErrorUI, WaitForExit, NeedProcessHandle, Admin, InheritAdmin, MostUsed.
- *dirEtc*  (`Au.Types.ROptions`):
  Allows to specify more parameters: current directory, verb, etc.
  If string, it sets initial current directory for the new process. If`""`, gets it from *file*. More info: `Au.Types.ROptions.CurrentDirectory`.
...
Uses API`ShellExecuteEx`.
Similar to`System.Diagnostics.Process.Start`.

The *file* parameter can be:

- Full path of a file or directory. Examples: `@"C:\file.txt"`, `folders.Documents`, `folders.System + "osk.exe"`, `@"%folders.System%\osk.exe"`.
- Filename of a file or directory, like `"abc.exe"`. The function calls `Au.filesystem.searchPath`.
- Path relative to `Au.folders.ThisApp`. Examples: `"abc.exe"`, `@"subfolder\abc.exe"`, `@".\subfolder\abc.exe"`, `@"..\another folder\abc.exe"`.
...
- To open a Windows Settings page can be used [ms-settings](🔗), like `"ms-settings:display"`. To open Settings use `"ms-settings:"`.
