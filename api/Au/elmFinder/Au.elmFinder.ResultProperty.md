...
The requested property of the found UI element, depending on `Au.elmFinder.ResultGetProperty`.
`null` if: 1. UI element not found. 2. `ResultGetProperty` not used or is `'-'`. 3. Failed to get the property.
