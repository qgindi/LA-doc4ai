...
- [Panels](🔗) contain multiple elements (controls, panels) and arrange them in a certain way (table, stack, etc). Panels allow to create UI layouts of any complexity.

Panel types:

- `Au.wpfBuilder.StartGrid` adds a [Grid](🔗) panel. It's a table with columns and rows. A cell usually contains 1 element, but can contain several or zero. An element can span several cells.
- `Au.wpfBuilder.StartStack` adds a [StackPanel](🔗). It arranges elements in a horizontal or vertical line.
- `Au.wpfBuilder.StartDock` adds a [DockPanel](🔗). It docks elements by its edges specified with `Au.wpfBuilder.Dock`. The last added element fills the remaining space.
- `Au.wpfBuilder.StartCanvas` adds a [Canvas](🔗) panel. It does not move/resize elements; for it use `Au.wpfBuilder.XY`.
