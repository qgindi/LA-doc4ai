...
Class `Au.wpfBuilder` allows to create [WPF](🔗) dialog windows quite easily without a visual designer. To start creating a dialog, use snippet `wpfDialogSnippet`: type `wpf` and select from the list. Or menu **File > New > Dialogs**.
...
WPF elements are of 2 main types: controls (visible elements like button, text box, label) and panels (containers for multiple elements). To add controls use `Au.wpfBuilder.Add` and other `AddX` functions. To add [panels](🔗) use `StartX` functions. Use other functions to set properties/content of the last added element.

Adding elements is like filling a table - in rows left-to-right and top-to-bottom. Usually don't need to set control position and size; for it you can define [columns and rows](🔗).
