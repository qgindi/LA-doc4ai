...
WPF elements (controls, panels) usually are inside panels of type `Grid`, `StackPanel` or `DockPanel`. These panels automatically set positions of child elements. Element positions and sizes in `Grid` depend on [columns and rows](🔗). In auto-sized columns/rows elements have default size or content size.
