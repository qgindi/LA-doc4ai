...
Regular expressions are used to parse text: compare, find/get substrings, find-replace, split. Use when you can't get the desired result with [simple string functions](🔗) easily.

This tool can help to create regular expressions: menu **Code > Regex**. It's like another cookbook.

Although .NET has a regular expression engine (class [Regex](🔗)), the automation library uses another engine - [PCRE](🔗). It's because some library functions can't use .NET. Both engines are equally powerful and use the same syntax with few unique features. Both can be used in scripts, but with `Au.elm` functions only PCRE.

The PCRE regular expression class is `Au.regexp`. And there are string extension methods that use it internally.

- `Au.regexp.IsMatch` and [string.RxIsMatch](🔗) - compare the string or find a substring.
- `Au.regexp.Match` and [string.RxMatch](🔗) - compare/find, and get match info.
- `Au.regexp.FindAll` and [string.RxFindAll](🔗) - find/get all matches.
- `Au.regexp.Replace` and [string.RxReplace](🔗) - find and replace all.
- `Au.regexp.Split` and [string.RxSplit](🔗) - split.
