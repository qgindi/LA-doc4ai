...
The second argument in the first example is a callback function, also known as [lambda](🔗). It is passed to `Func1`, and then `Func1` calls it.
...
The above examples use generic function types [Action\[\[lt\]\]T\[\[gt\]\]](🔗) and [Func\[\[lt\]\]T, TResult\[\[gt\]\]](🔗). Function types can be defined with the `delegate` keyword.
...
Another way is *events*. An event can have multiple subscribed functions. When raised, it calls all them. Subscribe with operator +=. Unsubscribe (if need) with operator -=. [More info](🔗).
...
Events are very useful in custom dialog windows. Examples in recipe [Dialog - events](🔗).