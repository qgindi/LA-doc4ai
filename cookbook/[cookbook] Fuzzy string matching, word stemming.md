...
Use fuzzy string matching to compare or find approximately matching words, for example misspelled. We'll use library [FuzzySharp](🔗). NuGet: FuzzySharp.

Use word stemming (remove suffix) to find all word forms (with/without any suffix). We'll use library [porter2-stemmer](🔗). NuGet: Porter2Stemmer. English only.
