...
Use `Au.filesystem` functions `Au.filesystem.enumerate`, `Au.filesystem.enumFiles`, `Au.filesystem.enumDirectories`. Tool: menu **Code > Get files in folder**.
...
Also can be used .NET classes [Directory](https://www.google.com/search?q=System.IO.Directory+class) (when need only paths), `DirectoryInfo` (when also need properties) or namespace `System.IO.Enumeration` (when need custom filtering).
