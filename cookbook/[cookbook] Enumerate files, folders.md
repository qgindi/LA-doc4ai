...
Use `Au.filesystem` functions `Au.filesystem.enumerate`, `Au.filesystem.enumFiles`, `Au.filesystem.enumDirectories`.
...
Also can be used .NET classes [Directory](🔗) (when need only paths), `DirectoryInfo` (when also need properties) or namespace `System.IO.Enumeration` (when need custom filtering).
