...
Examples of file and directory (folder) [paths](https://www.google.com/search?q=File+path+formats+on+Windows).
...
Use class `Au.folders` to get paths of special folders.
...
Special folders and environment variables can be in path string. Some functions support such strings; for others need `Au.pathname.expand`.
...
Use class `Au.pathname` or [Path](https://www.google.com/search?q=System.IO.Path+class) to manipulate path strings.
