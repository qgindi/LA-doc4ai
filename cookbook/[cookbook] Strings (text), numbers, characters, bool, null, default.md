...
Strings contain text. More info: [C# strings](🔗), recipe [strings and characters](🔗).
...
To easily create strings with variables, can be used [interpolated strings](🔗), operator `+` and other ways. More info in recipe [string formatting](🔗).
...
[Integer numbers](🔗) are numbers like `10` but not like `1.5`.
...
[Floating-point numbers](🔗) can be non-integer and can hold larger values.
