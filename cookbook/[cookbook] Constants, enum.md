...
Constants are like variables with a value that can't be changed. Also they can be used where variables can't, for example with `case` and as default parameter values. [More info](🔗).
...
The [`enum`](🔗) keyword declares multiple integer constants and gives them a type. Physically they are like `int` by default.
