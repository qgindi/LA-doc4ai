...
Various .NET and automation library classes and functions can be used to implement triggers and events of all kinds. For example `SystemEvents`, `FileSystemWatcher`, `Au.More.WindowsHook`, `Au.More.WinEventHook`, `Au.process.triggers`. A good place for triggers that should be active all the time is file `Other triggers`. To open it, use menu **TT > Other triggers**. This is an `Other triggers` file example.
